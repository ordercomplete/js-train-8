...
function getUsersFromDatabase(array) {
  return Array.from(array, ({ id, name, age }) => ({
    id,
...
function createArray(...elements) {
  if (elements.length === 0) {
    console.error("Не передано жодного елементу.");
    return [];
  }
...
/**
 * Копіюємо та переставляємо задану кількість елементів з масиву в нове місце за допомогою методу `copyWithin`.
...
function customUnshift(arr, ...elements) {
  if (!Array.isArray(arr)) {
    return "Вхідний параметр повинен бути масивом";
  }
...
function customIncludes(arr, element) {
  if (!Array.isArray(arr)) {
    return "Перший параметр повинен бути масивом";
  }
